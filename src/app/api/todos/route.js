...
export const DELETE = async (req) => {
    try {
        ...
        const del = await todoSchema.findByIdAndDelete({ _id: id })
        if (!del) return NextResponse.json({ success: false, message: "Invalid Error" })
        return NextResponse.json({ success: true, message: "Successfully Delete" })


    } catch (err) {
        return NextResponse.json({ success: false, err: "Error While Delete" })
    }
}